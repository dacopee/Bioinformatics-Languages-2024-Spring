...
mkdir 'DCProcessedGenes';

$dir = "YeastGenesA"; 
opendir(DIR,$dir) or die "can't open directory $dir:$!"; #open the dir with yeast genes

while ($filename = readdir DIR){ # loop through all alignment files
...
open (OUTFILE, ">"."./DCProcessedGenes/"."$filename") || die " could not open output file\n"; 
...
my $protein_sequence= '';
    ...
    print OUTFILE "DNA Sequence:\n", $DNA, "\n";
    print OUTFILE "GC Percentage: ", $GCPCT, "%";
    print OUTFILE "Protein Sequence:\n", $protein_sequence, "\n";
}                 
close(OUTFILE); 
}  # end while loop
